...
current_player=$(playerctl -l | grep -E 'spotify|firefox|brave|zen' | while read player; do
...
if [[ "$current_player" == "spotify" ]]; then
	icon="  " # Spotify icon
elif [[ "$current_player" == "firefox" || "$current_player" == "brave" || "$current_player" == "zen" ]]; then
	icon="  " # YouTube icon
else
