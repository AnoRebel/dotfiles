#!/usr/bin/env python3
"""
FocusTime stats retriever — queries historical data from the focustime database.
Used by the QML UI for browsing past days and per-app drill-down.

Usage: python3 get_stats.py [YYYY-MM-DD] [--app APP_CLASS]
"""
import sqlite3
import json
import os
...
def main():
    ...
    if not os.path.exists(DB_PATH):
        print(json.dumps({
            "total": 0, "average": 0, "week_range": "", "yesterday": 0,
            "current": "History", "apps": [], "week_apps": [], "week": [],
            "month": [], "hourly": [0] * 48,
    ...
    c.execute(f"""SELECT COUNT(DISTINCT log_date), SUM(seconds)
        FROM focus_log WHERE log_date >= ? AND log_date <= ? AND seconds > 0 {filter_sql}""", params_avg)
    row = c.fetchone()
    ...
    c.execute(f"""SELECT app_class, COALESCE(app_title, app_class), SUM(seconds) as secs
        FROM focus_log WHERE log_date = ?{filter_sql} GROUP BY app_class ORDER BY secs DESC""", params)
    all_apps = []
    for app_class, app_title, secs in c.fetchall():
        percentage = (secs / total_seconds) * 100 if total_seconds > 0 else 0
        all_apps.append({
            "class": app_class, "name": app_title, "icon": get_app_icon(app_class),
            "seconds": secs, "percent": round(percentage, 1),
        })

    c.execute(f"""SELECT app_class, COALESCE(app_title, app_class), SUM(seconds) as secs
        FROM focus_log WHERE log_date >= ? AND log_date <= ? {filter_sql}
        GROUP BY app_class ORDER BY secs DESC LIMIT 50""", params_avg)
    ...
    week_heatmap = [[0] * 24 for _ in range(7)]
    try:
        c.execute(f"""SELECT log_date, hour, SUM(seconds) FROM focus_hourly
            WHERE log_date >= ? AND log_date <= ? {filter_sql} GROUP BY log_date, hour""", params_avg)
    ...
    minute_data = [0] * 1440
    try:
        c.execute(f"""SELECT minute_idx, SUM(seconds) FROM focus_minutes
            WHERE log_date >= ? AND log_date <= ? {filter_sql} GROUP BY minute_idx""", params_avg)
        for idx, secs in c.fetchall():
            if 0 <= idx < 1440:
