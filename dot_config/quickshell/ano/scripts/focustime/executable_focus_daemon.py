#!/usr/bin/env python3
"""
FocusTime daemon — tracks active window usage per application.
Compositor-agnostic: supports Hyprland (IPC) and Niri (niri msg).
Adapted from ilyamiro's focustime for Ano Shell.

Usage: python3 focus_daemon.py
Data: ~/.local/share/focustime/focustime.db
State: $XDG_RUNTIME_DIR/focustime_state.json (live, polled by QML)
"""
import subprocess
import sqlite3
import time
...
if os.environ.get("HYPRLAND_INSTANCE_SIGNATURE"):
    ...
    COMPOSITOR = "niri"
elif os.environ.get("XDG_CURRENT_DESKTOP", "").lower() == "niri":
    COMPOSITOR = "niri"
...
def get_xdg_search_dirs():
    ...
    for d in ["/var/lib/flatpak/exports/share/applications", "/var/lib/snapd/desktop/applications"]:
...
def resolve_app_name(app_class, raw_title):
    ...
    base_class = re.sub(r"[-_ ]?updater$", "", app_class_lower).replace(".exe", "")
...
def get_active_window_hyprctl():
    try:
        ...
        return app_cls, resolve_app_name(app_cls, raw_title)
    except Exception:
        return "Unknown", "Unknown"


def is_locked_hyprland():
    try:
            ...
            while True:
                ...
                while "\n" in buffer:
                    ...
                    if line.startswith("activewindow>>"):
                        ...
                        if is_locked_hyprland() or cls == "hyprlock":
...
def get_active_window_niri():
    try:
        ...
        if "quickshell" in app_id.lower():
            return "Quickshell", "Quickshell"
        app_id = app_id if app_id else "Unknown"
        title = title if title else app_id
        return app_id, resolve_app_name(app_id, title)
    except Exception:
        return "Unknown", "Unknown"
...
def dump_state_to_json(c):
    ...
    c.execute(
        ...
        FROM focus_log WHERE log_date >= ? AND log_date <= ? AND seconds > 0""",
    ...
    c.execute(
        ...
        FROM focus_log WHERE log_date = ? GROUP BY app_class ORDER BY secs DESC""",
    ...
    c.execute(
        """SELECT app_class, COALESCE(app_title, app_class), SUM(seconds) as secs
        FROM focus_log WHERE log_date >= ? AND log_date <= ?
        GROUP BY app_class ORDER BY secs DESC LIMIT 50""",
        (monday.isoformat(), sunday.isoformat()),
    )
    week_apps_rows = c.fetchall()
    week_apps_total = sum(r[2] for r in week_apps_rows)
    week_apps = []
    for cls, title, secs in week_apps_rows:
    ...
    # Hourly breakdown (48 half-hour blocks)
    hourly_data = [0] * 48
    try:
    ...
    try:
        c.execute(
            """SELECT log_date, hour, SUM(seconds) FROM focus_hourly
            WHERE log_date >= ? AND log_date <= ? GROUP BY log_date, hour""",
            (monday.isoformat(), sunday.isoformat()),
        )
    ...
    # Peak usage (sliding 60-minute window over minute-level data)
    minute_data = [0] * 1440
    try:
        c.execute(
            """SELECT minute_idx, SUM(seconds) FROM focus_minutes
            WHERE log_date >= ? AND log_date <= ? GROUP BY minute_idx""",
            (monday.isoformat(), sunday.isoformat()),
        )
        for idx, secs in c.fetchall():
            if 0 <= idx < 1440:
...
def main():
    ...
    if COMPOSITOR == "hyprland":
        ...
        ipc_thread = threading.Thread(target=listen_niri_events, daemon=True)
        ipc_thread.start()
    else:
        print(f"[focustime] Unknown compositor, polling hyprctl/niri msg")
        current_app_class, current_app_title = "Desktop", "Desktop"
    ...
    while True:
        ...
        if current_app_class and current_app_class != "":
            ...
            c.execute(
                ...
                VALUES (?, ?, ?, 1)
                ON CONFLICT(log_date, hour, app_class) DO UPDATE SET seconds = seconds + 1""",
            ...
            c.execute(
                ...
                VALUES (?, ?, ?, 1)
                ON CONFLICT(log_date, interval_idx, app_class) DO UPDATE SET seconds = seconds + 1""",
            ...
            c.execute(
                ...
                VALUES (?, ?, ?, 1)
                ON CONFLICT(log_date, minute_idx, app_class) DO UPDATE SET seconds = seconds + 1""",
