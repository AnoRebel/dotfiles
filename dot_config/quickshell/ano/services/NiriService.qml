...
// NiriService: Full Niri compositor IPC service via socket.
// Provides workspace management, window tracking, tiling layouts,
// keyboard layout switching, and comprehensive action dispatch.
//
// Ported from @inir/ NiriService with adaptations for Ano:
// - Uses AnoSocket instead of DankSocket
// - Removed GameMode dependency (will be added later)
// - Fixed update intervals to sensible defaults
//
// Extension point: A C++ plugin could provide a native Niri module
// similar to Quickshell.Hyprland for better integration.
Singleton {
    id: root
    ...
    // Window list update interval (ms) — fixed defaults, no GameMode dependency yet
    readonly property int windowListUpdateIntervalMs: 50
    ...
    function handleOutputsChanged(data) {
        ...
        updateDisplayScales()
        // Force immediate update for outputs as it affects geometry calculations
        windows = sortWindowsByLayout(windows)
    }
    ...
    function _applyMasterLeft(count): void {
        if (count < 2) return
        // Master in col 0 (50%), stack in col 1
        focusColumnFirst()
        setColumnWidth("50%")
        focusColumnRight()
        for (let i = 0; i < count - 2; i++) {
    ...
    function _applyMasterRight(count): void {
        if (count < 2) return
        // Stack in col 0, Master in col 1 (50%)
        focusColumnFirst()
        for (let i = 0; i < count - 2; i++) {
            consumeWindowIntoColumn()
        }
